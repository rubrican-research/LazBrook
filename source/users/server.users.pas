...
type
    ...
    NLazBrookLogoutState = (
          ...
          logout_state_CheckSessionIDExists,
          logout_state_CheckSessionValid,
      ...
      NLazBrookCanLoginResult = (
          lrAllOk,
          lrUserNotExists,
          lrUserOnboarding,
          lrUserNotActive,        // This is the status of the user
          lrUserAccountNotActive  // This is the status of the user's account.
      );

      NLazBrookUserLoginStatus = (
          ulsUnknown,
          ulsLoggedOut,
          ulsWrongCredentials,
          ulsExceededRetries,
    ...
    private
		function getAttemptsRemaining: byte;
    ...
    published
        property AttemptsRemaining: byte read getAttemptsRemaining;
    ...
    public
        ...
        function decAttemptsRemaining: byte;
    end;
    ...
    private
		...
		function getLoginStatus: NLazBrookUserLoginStatus;
		...
		procedure setLoginStatus(const _value: NLazBrookUserLoginStatus);
    ...
    published
        ...
        property loginStatus : NLazBrookUserLoginStatus read getLoginStatus write setLoginStatus;
    public
        constructor Create; override;
    ...
    private
		...
		function getUser(_i: DWord): TLazBrookUser;
    ...
    public
        ...
        procedure reset; // Clears everything
        ...
        function isTimedout(_loginID: string): boolean;

        function canLogin(_loginID: string; out _reason: NLazBrookCanLoginResult): boolean;
        function verifyCredentialsGetUser(const _loginID: string; const _pwd: string): TLazBrookUser;
        function doLogin(const _loginID: string; const _pwd: string): NLazBrookUserLoginStatus;
        ...
        function getLoginStatus(_loginID: string): NLazBrookUserLoginStatus;
    ...
    published
        ...
        property count : DWord read getUserCount;
    public
        property Items[_i: DWord] : TLazBrookUser read getUser;
    end;
...
uses
    sugar.utils, sugar.jsonlib, Math, DateUtils;
...
function TLazBrookUserAuth.getAttemptsRemaining: byte;
begin
    Result := integers['AttemptsRemaining'];
end;
...
function TLazBrookUserAuth.decAttemptsRemaining: byte;
begin
    Result := integers['AttemptsRemaining'];
    If Result > 0 then begin
        Dec(Result);
        integers['AttemptsRemaining'] := Result;
	end;
end;
...
procedure TLazBrookUserAuth.reset;
begin
    Strings['PwdHash']      := '';
    Strings['Salt']         := '';
...
procedure TLazBrookUserAuth.resetAttempts;
begin
    integers['AttemptsRemaining'] := MAX_LOGIN_ATTEMPTS;
end;
...
function TLazBrookLoginManager.getUser(_i: DWord): TLazBrookUser;
begin
    Result := usersByLoginId.Data[_i];
end;

function TLazBrookLoginManager.getUserCount: DWord;
begin
    Result := usersByLoginId.Count;
end;
...
procedure TLazBrookLoginManager.reset;
begin
    {These are indices. Only references}
    myUsersBySession.Clear;
    myUsersOnBoarding.Clear;
    myUsersLoggedIn.Clear;
    myUsersLoggedOut.Clear;
    myUsersByLoginId.Clear;
end;
...
function TLazBrookLoginManager.isTimedout(_loginID: string): boolean;
var
	_user: TLazBrookUser;
begin
    _user := user(_loginID) ;
    if assigned(_user) then
        Result := (_user.Session.expires <> 0) and (_user.Session.expires < Now)
    else
        Result := false;
end;
...
function TLazBrookLoginManager.canLogin(_loginID: string; out
	_reason: NLazBrookCanLoginResult): boolean;
var
    _user: TLazBrookUser;
        ...
        case _stage of
            ...
            NStageCheckLogin.isActive: begin
                if not _user.isActive then begin
                    _reason := lrUserNotActive;
                    inc(_error);
                    _stage := NStageCheckLogin.done;
                    continue;
                end;

                if not _user.Auth.isActive then begin
                    _reason := lrUserAccountNotActive;
                    inc(_error);
				end;
...
function TLazBrookLoginManager.doLogin(const _loginID: string;
	const _pwd: string): NLazBrookUserLoginStatus;
var
    _user, _loginUser: TLazBrookUser;
begin
{
NLazBrookUserLoginStatus = (
        ulsUnknown,
    ...
    if isLoggedIn(_loginID) then begin
        if not isTimedOut(_loginID) then
            Result := ulsLoggedIn
        else begin
            doLogout(_loginID);
            Result := ulsTimedOut;
    ...
    if assigned(_user) then
    begin
        _loginUser := verifyCredentialsGetUser(_loginID, _pwd);
        if assigned(_loginUser) then begin
            _loginUser.Session.initialize;  // Generates a new session ID and sets begin time and expiry time
            _loginUser.auth.resetAttempts; // Resets the count of available login attempts

            usersBySession.Add(_loginUser.Session.sessionID, _loginUser); // stores sessionID -> user mapping
            usersLoggedOut.Remove(_loginID);    // removes this user from list of loggout users
            usersLoggedIn.Add(_loginID, _loginUser); // adds this user to list of logged in users
            Result := ulsLoggedIn;
            _loginUser.Session.loginStatus:= Result;
		end
        else begin
            if _user.auth.decAttemptsRemaining > 0 then begin
                Result := ulsWrongCredentials;
			end
            else begin
                Result := ulsExceededRetries;
			end;
            _user.Session.loginStatus := Result;
		end;
	end
    else begin
        Result := ulsWrongCredentials;
        ...
        case state of
            ...
            logout_state_CheckUserLoggedIn: begin
                ...
                if _indexLoggedInUser > -1 then
                    state := logout_state_CheckSessionIDExists
                else
                begin
                    state := logout_state_Error;
            ...
            logout_state_CheckSessionIDExists: begin
                _sessionID := _user.session.sessionID;
                if Length(_sessionID) > 0 then
                    state := logout_state_CheckSessionValid
                else
                begin
                    state := logout_state_Error;
            ...
            logout_state_CheckSessionValid: begin
                _indexUserBySession := usersBySession.IndexOf(_user.session.sessionID);
            ...
            logout_state_PerformLogout: begin
                usersBySession.Remove(_user.session.sessionID);
                usersLoggedIn.Remove(_loginID);
                usersLoggedOut.Add(_loginID, _user);
                _user.session.reset;
                _user.session.loginStatus := ulsLoggedOut;
...
function TLazBrookLoginManager.getLoginStatus(_loginID: string
	): NLazBrookUserLoginStatus;
var
	_usr: TLazBrookUser;
begin
    Result := ulsUnknown;
    _usr := user(_loginID);
    if assigned(_usr) then begin
        Result := _usr.Session.loginStatus;
	end;
end;
...
function TLazBrookUserSession.getLoginStatus: NLazBrookUserLoginStatus;
begin
    Result := NLazBrookUserLoginStatus(integers['loginStatus']);
end;
...
procedure TLazBrookUserSession.setLoginStatus(
	const _value: NLazBrookUserLoginStatus);
begin
    integers['loginStatus'] := ord(_value);
end;
...
begin
    ...
    Floats['Expires'] := 0;
    Strings['SessionID'] := '';
    Integers['loginStatus'] := 0;
end;

procedure TLazBrookUserSession.initialize;
const
...
begin
    CreateGUID(_guid);
    _now := Now();
    Floats['LoginTime'] := _now;
    Floats['LastReqAt'] := _now;
    Floats['Expires']   := IncSecond(_now, DEFAULT_SESSION_DURATION);
    Strings['SessionID']:= _guid.ToString(true);
end;
